     ...
     obj.charges(i,5)=0;
      ...
      if(isempty(obj.get_W_value(material))) %if charge cannot drift in the material
       ...
       % New position= last position + v.dt -> with v oriented along E
		...
		%COSICAF V2: charge multiplication
%		alpha=townsend_ionisation(obj,medium,E,reducedE)
		if(isempty(obj.townsend_data)==0)
		alpha=obj.townsend_ionisation(material,norm(E),obj.compute_reduced_Efield(norm(E),material,dens(1)));
		
			if(alpha~=0.0)
			obj.charges(i,5)=obj.charges(i,5)+obj.charges(i,5)*alpha*v*obj.time_step;
			%%to add-> ion generation
			end
